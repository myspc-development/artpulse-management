...
class PendingArtworks {

    public static function register() {
        // Hooks to handle custom approve/reject actions
        add_action('admin_init', [self::class, 'handle_artwork_actions']);
        add_action('admin_init', [self::class, 'handle_reject_artwork']);
    }

    /**
     * Handle custom actions like 'approve' for single artworks from the pending list.
     */
    public static function handle_artwork_actions() {
        // Check if our specific action is being performed
        if (
            isset($_GET['page']) && $_GET['page'] === 'artpulse-pending-artworks' && // Ensure it's from our page
            ...
            // Verify nonce
            if (!wp_verify_nonce($nonce, 'ead_approve_artwork_' . $post_id)) {
                wp_die(__('Security check failed. Please try again.', 'artpulse-management'));
            }

            // Check user capabilities
            if (!current_user_can('publish_post', $post_id)) {
                wp_die(__('You do not have permission to approve this artwork.', 'artpulse-management'));
            }
            ...
            if (is_wp_error($updated) || !$updated) {
                ...
                wp_redirect(admin_url('admin.php?page=artpulse-pending-artworks&approval_error=1&post_id=' . $post_id));
            } else {
                ...
                wp_redirect(admin_url('admin.php?page=artpulse-pending-artworks&artwork_approved=1&post_id=' . $post_id));
            }
            exit;
        }
    }
    ...
    /**
     * Handle single artwork rejection (move to trash).
     */
    public static function handle_reject_artwork() {
        if (
            isset($_GET['page']) && $_GET['page'] === 'artpulse-pending-artworks' &&
            ...
            if (!wp_verify_nonce($nonce, 'ead_reject_artwork_' . $post_id)) {
                wp_die(__('Security check failed. Please try again.', 'artpulse-management'));
            }

            if (!current_user_can('delete_post', $post_id)) {
                wp_die(__('You do not have permission to reject this artwork.', 'artpulse-management'));
            }

            $trashed = wp_trash_post($post_id);

            if (is_wp_error($trashed) || !$trashed) {
                wp_redirect(admin_url('admin.php?page=artpulse-pending-artworks&rejection_error=1&post_id=' . $post_id));
            } else {
                wp_redirect(admin_url('admin.php?page=artpulse-pending-artworks&artwork_rejected=1&post_id=' . $post_id));
            }
            exit;
        }
    }
    ...
    /**
     * Render the Pending Artworks page.
     */
    public static function render_admin_page() {
        self::handle_bulk_actions();

        // Display admin notices for actions
        if (isset($_GET['artwork_approved']) && (int) $_GET['artwork_approved'] && isset($_GET['post_id'])) {
            printf(
                '<div class="notice notice-success is-dismissible"><p>%s</p></div>',
                sprintf(esc_html__('Artwork "%s" (ID: %d) has been approved.', 'artpulse-management'), get_the_title(intval($_GET['post_id'])), intval($_GET['post_id']))
            );
        }
        if (isset($_GET['approval_error']) && isset($_GET['post_id'])) {
            printf(
                '<div class="notice notice-error is-dismissible"><p>%s</p></div>',
                sprintf(esc_html__('Could not approve artwork (ID: %d).', 'artpulse-management'), intval($_GET['post_id']))
            );
        }
        if (isset($_GET['artwork_rejected']) && (int) $_GET['artwork_rejected'] && isset($_GET['post_id'])) {
            printf(
                '<div class="notice notice-warning is-dismissible"><p>%s</p></div>',
                sprintf(esc_html__('Artwork "%s" (ID: %d) has been rejected and moved to trash.', 'artpulse-management'), get_the_title(intval($_GET['post_id'])), intval($_GET['post_id']))
            );
        }
        if (isset($_GET['rejection_error']) && isset($_GET['post_id'])) {
            printf(
                '<div class="notice notice-error is-dismissible"><p>%s</p></div>',
                sprintf(esc_html__('Could not reject artwork (ID: %d).', 'artpulse-management'), intval($_GET['post_id']))
            );
        }
        if (isset($_GET['bulk_action_status']) && $_GET['bulk_action_status'] === 'none_selected') {
            echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__('No artworks were selected for the bulk action.', 'artpulse-management') . '</p></div>';
        }
        if (isset($_GET['approved']) && (int) $_GET['approved']) {
            printf('<div class="notice notice-success is-dismissible"><p>%s</p></div>',
                esc_html(sprintf(_n('%d artwork approved.', '%d artworks approved.', (int)$_GET['approved'], 'artpulse-management'), (int)$_GET['approved']))
            );
        }
        if (isset($_GET['rejected']) && (int) $_GET['rejected']) {
            printf('<div class="notice notice-warning is-dismissible"><p>%s</p></div>',
                esc_html(sprintf(_n('%d artwork rejected and moved to trash.', '%d artworks rejected and moved to trash.', (int)$_GET['rejected'], 'artpulse-management'), (int)$_GET['rejected']))
            );
        }


        echo '<div class="wrap">';
        echo '<h1>' . esc_html__('Pending Artworks', 'artpulse-management') . '</h1>';

        echo '<form method="post" id="ead-pending-artworks-form">';
        echo '<input type="hidden" name="page" value="' . esc_attr($_REQUEST['page'] ?? 'artpulse-pending-artworks') . '" />';
        wp_nonce_field('ead_pending_artworks_bulk_action_nonce', 'ead_pending_artworks_bulk_nonce');
        ...
        $pending_artworks = new \WP_Query($args);

        if ($pending_artworks->have_posts()) {
            ...
            while ($pending_artworks->have_posts()) {
                $pending_artworks->the_post();
                $post_id = get_the_ID();
                $title = get_the_title();
                ...
                // Approve button
                $approve_nonce = wp_create_nonce('ead_approve_artwork_' . $post_id);
                $approve_url = add_query_arg([
                    'page'    => 'artpulse-pending-artworks',
                    'action'  => 'approve',
                    'post_id' => $post_id,
                    'nonce'   => $approve_nonce
                ], admin_url('admin.php'));

                $reject_nonce = wp_create_nonce('ead_reject_artwork_' . $post_id);
            ...
            echo '<p>' . esc_html__('No pending artworks found.', 'artpulse-management') . '</p>';
        }
    ...
    /**
     * Handle bulk actions for pending artworks.
     */
    private static function handle_bulk_actions() {
        if (isset($_POST['ead_do_bulk_action'])) {
            check_admin_referer('ead_pending_artworks_bulk_action_nonce', 'ead_pending_artworks_bulk_nonce');

            $action = isset($_POST['ead_bulk_action']) ? sanitize_text_field($_POST['ead_bulk_action']) : '-1';
            $artwork_ids = isset($_POST['artwork_ids']) && is_array($_POST['artwork_ids']) ? array_map('intval', $_POST['artwork_ids']) : [];

            if (empty($artwork_ids)) {
                wp_redirect(admin_url('admin.php?page=' . ($_REQUEST['page'] ?? 'artpulse-pending-artworks') . '&bulk_action_status=none_selected'));
                exit;
            }
            ...
            if ($action === 'approve_selected') {
                if (!current_user_can('publish_posts')) {
                    wp_die(__('You do not have permission to approve artworks.', 'artpulse-management'));
                }
                foreach ($artwork_ids as $artwork_id) {
                    if (get_post_type($artwork_id) === 'ead_artwork' && get_post_status($artwork_id) === 'pending') {
                        wp_update_post(['ID' => $artwork_id, 'post_status' => 'publish']);
                        $approved_count++;
                    }
                }
            } elseif ($action === 'reject_selected') {
                if (!current_user_can('delete_posts')) {
                    wp_die(__('You do not have permission to reject artworks.', 'artpulse-management'));
                }
                foreach ($artwork_ids as $artwork_id) {
                    if (get_post_type($artwork_id) === 'ead_artwork') {
                        wp_trash_post($artwork_id);
                        $rejected_count++;
                    }
                }
            }

            $redirect_args = ['page' => $_REQUEST['page'] ?? 'artpulse-pending-artworks'];
            if ($approved_count > 0) {

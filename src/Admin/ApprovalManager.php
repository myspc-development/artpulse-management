...
class ApprovalManager
{
    ...
    public static function addApprovalMetabox()
    {
        ...
        foreach ($post_types as $pt) {
            add_meta_box(
                'ap-approval',
                __('Submission Approval', 'artpulse-management'),
    ...
    public static function renderMetabox($post)
    {
        if ('pending' !== $post->post_status) {
            echo '<p>' . esc_html__('This submission is already reviewed.', 'artpulse-management') . '</p>';
            return;
        }
        ...
        <form method="post" action="<?php echo esc_url($approve_url); ?>">
            ...
            <?php submit_button(__('Approve Submission', 'artpulse-management'), 'primary', 'submit', false); ?>
        </form>
        <?php
    }
    ...
    public static function handleApproval()
    {
        ...
        if ( ! wp_verify_nonce($nonce, 'ap_approve_' . $post_id) ) {
            wp_die(esc_html__('Security check failed', 'artpulse-management'));
        }

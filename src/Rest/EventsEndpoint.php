...
class SubmitEventEndpoint extends WP_REST_Controller {
    ...
    public function submitEvent( WP_REST_Request $request ) {
        ...
        if ( ! current_user_can( 'ead_submit_event' ) ) {
            return new WP_Error( 'rest_forbidden', __( 'You do not have permission to submit events.', 'artpulse-management' ), [ 'status' => 403 ] );
        }
        ...
        $featured        = rest_sanitize_boolean( $request->get_param( 'event_featured' ) ) ? '1' : '';
        $latitude        = $request->get_param( 'latitude' ) !== null ? floatval( sanitize_text_field( $request->get_param( 'latitude' ) ) ) : '';
        $longitude       = $request->get_param( 'longitude' ) !== null ? floatval( sanitize_text_field( $request->get_param( 'longitude' ) ) ) : '';
        ...
        if ( empty( $start_date ) ) {
            return new WP_Error( 'missing_start', __( 'Start date is required.', 'artpulse-management' ), [ 'status' => 400 ] );
        }
        if ( empty( $end_date ) ) {
            return new WP_Error( 'missing_end', __( 'End date is required.', 'artpulse-management' ), [ 'status' => 400 ] );
        }
        if ( strtotime( $start_date ) > strtotime( $end_date ) ) {
            return new WP_Error( 'invalid_dates', __( 'End date must be after start date.', 'artpulse-management' ), [ 'status' => 400 ] );
        }
        if ( empty( $organizer ) ) {
            return new WP_Error( 'missing_organizer', __( 'Organizer is required.', 'artpulse-management' ), [ 'status' => 400 ] );
        }
        ...
        if ( ! empty( $event_type ) && term_exists( intval( $event_type ), 'ead_event_type' ) ) {
        ...
        if ( $send_admin_notify ) {
            ...
            if ( class_exists( 'EAD\\Helpers\\EmailHelper' ) ) {
                $email_sent = EmailHelper::send_email(
                    $adminEmail,
                ...
                if ( ! $email_sent ) {
                    error_log( 'ArtPulse Management: Failed to send admin notification email via EmailHelper.' );
                }
            } else {
                error_log( 'ArtPulse Management: EmailHelper class not found.  Cannot send admin notification.' );
            }
        }
    ...
    public function permissionsCheck( WP_REST_Request $request ) {
        return is_user_logged_in() && current_user_can( 'ead_submit_event' );
    }

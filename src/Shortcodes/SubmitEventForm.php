...
class SubmitEventForm {
    ...
    public static function render( $atts, $content = null ) {
        if ( ! is_user_logged_in() ) {
            return '<div class="ead-notice ead-login-required">' . esc_html__( 'You must be logged in to submit an event.', 'artpulse-management' ) . '</div>';
        }
        ...
        <div class="ead-dashboard-card ead-submit-event-wrapper">
            <form id="ead-submit-event-form" enctype="multipart/form-data" autocomplete="off">
                <?php wp_nonce_field( 'ead_submit_event_form_display', 'ead_submit_event_form_nonce' ); ?>
                <div id="ead-submit-event-message" class="ead-submit-event-message" style="display:none;"></div>

                <label for="ead_event_title"><?php esc_html_e( 'Event Title*', 'artpulse-management' ); ?></label>
                <input type="text" id="ead_event_title" name="title" required />

                <label for="ead_event_description"><?php esc_html_e( 'Description*', 'artpulse-management' ); ?></label>
                <textarea id="ead_event_description" name="description" required></textarea>

                <label for="ead_event_type"><?php esc_html_e( 'Event Type*', 'artpulse-management' ); ?></label>
                ...
                <label for="ead_event_start_date"><?php esc_html_e( 'Start Date*', 'artpulse-management' ); ?></label>
                <input type="date" id="ead_event_start_date" name="event_start_date" required />

                <label for="ead_event_end_date"><?php esc_html_e( 'End Date*', 'artpulse-management' ); ?></label>
                <input type="date" id="ead_event_end_date" name="event_end_date" required />

                <label for="ead_venue_name"><?php esc_html_e( 'Venue Name', 'artpulse-management' ); ?></label>
                ...
                <div class="ead-address-fields">
                    <label><strong><?php esc_html_e( 'Event Address', 'artpulse-management' ); ?></strong></label>
                    ...
                    <div class="ead-map-controls">
                        <button type="button" id="ead-geolocate-btn"><?php esc_html_e( 'Use My Location', 'artpulse-management' ); ?></button>
                    </div>
                    <div id="ead-map" style="width:100%;height:260px;margin-top:1rem;display:none;"></div>
                </div>

                <label for="ead_event_organizer"><?php esc_html_e( 'Organizer', 'artpulse-management' ); ?></label>
                <input type="text" id="ead_event_organizer" name="organizer" />

                <label for="ead_event_organizer_email"><?php esc_html_e( 'Organizer Email', 'artpulse-management' ); ?></label>
                ...
                <button type="submit" id="ead-submit-event-button"><?php esc_html_e( 'Submit Event', 'artpulse-management' ); ?></button>
                <span id="ead-submit-event-loading" class="ead-submit-event-loading" style="display:none;"><?php esc_html_e( 'Submitting...', 'artpulse-management' ); ?></span>
            </form>
        </div>
        <?php
        return ob_get_clean();

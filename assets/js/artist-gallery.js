jQuery(document).ready(function ($) {
    $('.ead-upload-image-button').on('click', function (e) {
        ...
        const frame = wp.media({
            title: eadArtistGallery.select_image_title || 'Select Image',
            button: { text: eadArtistGallery.use_image_button || 'Use this image' },
            multiple: false,
            library: { type: 'image' }
        });
        ...
        frame.on('select', function () {
            ...
            $container.trigger('eadImageSelected', [attachment]);
        });
    ...
    $('.ead-remove-image-button').on('click', function (e) {
        ...
        $preview.css('background-image', '').removeClass('has-image')
                .html('<span class="placeholder">' + (eadArtistGallery.placeholder_prefix || 'Image ') + (index + 1) + '</span>');
        $(this).addClass('hidden');
        $container.trigger('eadImageRemoved');
    });
});
